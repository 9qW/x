...
client.on("reachLimit", (limit)=> {
  ...
  let log = limit.guild.channels.find( channel => channel.name === "rle");
  const loghack = new Discord.RichEmbed()
       .setAuthor(`${limit.user.tag}`, limit.user.avatarURL)
       .setColor('#36393e')
       .setDescription(` <@${limit.user.id}> Trying to Hack and i removed all his roles .  `)
       .setTimestamp();
  log.send(loghack);
 // log.send("");
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "setgame")) {
    ...
    message.channel.sendMessage(white_check_mark Done Changed Name To ${argresult});
...
client.on('guildMemberUpdate', (oldMember, newMember) => {
  ...
  let loga = "736963101005709312"
  let role1 = "" // ايدي رولات ؛
  let role2 = ""
  let role3 = ""
    ...
    oldMember.guild.fetchAuditLogs().then(logs => {
     ...
     if(userID == '681467216474275869'
      || userID == '733229125887721532'
       ) return;
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("737559816368685068");
    if (!channel) {
 ...
 channel.sendMessage(`Syned By ; ${Invite.inviter} `) ;         
 }
            dat[Inv] = Invite.uses;
...
client.on('guildMemberUpdate', (oldMember, newMember) => {
    if(!oldMember.guild) return;
  let loga = "737592080905404417"
  let role1 = "738334634240114693"
  let role2 = "735177222331170867"
  let role3 = "734724848395354202"
  let role4 = "738343072466731058"
  let role5 = "" // Rr.#1835
  let role6 = ""
  let role7 = "" // Rr.#1835
  let role8 = "" // Rr.#1835
  ...
  let role23 = "" // Rr.#1835
    ...
    oldMember.guild.fetchAuditLogs().then(logs => {
        var userID = logs.entries.first().executor.id;
     if(oldMember.roles.size < newMember.roles.size) {
     if(userID == ''     // ايديات الي ييقدرون يعطون هلرتبة
...
var kboosh = new Eris("NzMyMTg4NDY3MDUxNzU3NjM4.XwxSPQ.tpaAAbjcMpIg6BOvii0mKE5vOoM");
var kboosh_id = "731469817328173166";
                    var i = "0";
                    var x = "0";
...
kboosh.on("messageCreate", (msg) => {
    if(msg.author.id !== "681467216474275869") return kboosh.createMessage('__**This Command is only for the bot Owner**__');
...
kboosh.connect("NzMyMTg4NDY3MDUxNzU3NjM4.XwxSPQ.tpaAAbjcMpIg6BOvii0mKE5vOoM")
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@681467216474275869>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
/*client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "settings")) {
    ...
    if (message.content.startsWith(prefix + "limitban")) {
...
*/
    message.author.sendEmbed(`مدة رابط يوم كامل ول ٥ اشخاص فقط `);
  }
});
/*
////لايحتاج تعديل
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setVc")) {
    ...
    channelfind.setName(
      `actv : ${message.guild.members.filter(m => m.voiceChannel).size}` ///تعديل غير اساسي تعديل اسم روم الفويس اونلاين
    );
